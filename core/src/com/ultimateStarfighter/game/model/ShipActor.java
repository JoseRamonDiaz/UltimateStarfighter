...
public class ShipActor extends Actor implements Disposable {
	...
	public ShipActor() {
		shipAnimation = new Animation(animSpeed, Services.getImgVector(path,
				numOfImgs));
		isEnemyShip = false;
		setX(100);
		setY(100);
		loadFrame();
		shipEngineSound = Gdx.audio.newSound(Gdx.files
				.internal("sounds/spaceShipEngine.wav"));
	...
	public ShipActor(String path, int x, int y) {
		...
		numOfImgs = 6;
		animSpeed = 0.05f;
		shipAnimation = new Animation(this.animSpeed, Services.getImgVector(
				this.path, numOfImgs));
		isEnemyShip = true;
		loadFrame();
	}
	...
	@Override
	public void dispose() {
		if(shipEngineSound != null)
			shipEngineSound.dispose();
	}
	...
	private void updateShipPos() {
		...
		if (posX > 0
				&& posX < this.getStage().getWidth() - frame.getRegionWidth())
			setX(posX);
		float posY = getY() + (control.getPercentY() * speed);
		if (posY > 0
				&& posY < this.getStage().getHeight() - frame.getRegionHeight())
			setY(posY);
	}
